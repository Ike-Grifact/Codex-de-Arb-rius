...
const requiredSources = [
  "01_Guia_Jogador_Lucker.html",
  "02_Guia_Jogador_Arboriano.html",
...
for (const source of requiredSources) {
  await access(resolve(sourceRoot, source));
}
...
const contentFiles = (await walk(resolve(root, "src/content"))).filter((file) =>
...
for (const file of contentFiles) {
  ...
  const visibility =
    source.match(/^visibility:\s*["']?([^"'\r\n]+)/m)?.[1]?.trim() || "publico";
  const status =
    source.match(/^status:\s*["']?([^"'\r\n]+)/m)?.[1]?.trim() || "publicado";

  if (!slug) throw new Error(`Metadado slug ausente em ${file}`);
  if (visibility === "publico" && status !== "rascunho") {
...
console.log(
  `Conteúdo válido: ${contentFiles.length} fontes, ${publicSlugs.size} slugs públicos e nenhum reservado em public.`
);
